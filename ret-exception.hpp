...
class Ret_except {
    ...
    template <class T>
    static constexpr bool holds_alternative_v = (std::is_same_v<T, Ts> || ...);

public:
    Ret_except() = default;

    template <class T, class ...Args, 
              class = std::enable_if_t<holds_alternative_v<T> && std::is_constructible_v<T, Args...>>>
    ...
    {
       if (!is_exception_handled && !v.valueless_by_exception())
